   ...
   def void apply(Project project) {
      project.task('updateTheVersion') {
         ...
         try {
            ...
            // Make sure this isn't just a single directory before attempting to isolate
            //   an app directory
            ...
            if (checkForGradlePropertiesInBase ) {
               File gradleBaseFile = new File("${project.projectDir}/gradle.properties")
               if (gradleBaseFile.exists()) {
                  gradlePropertiesInBase = true
               }
               else {
                  println "INFO: No gradle.properties found in base root project '${rootPath}'"
